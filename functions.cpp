...
void print_matrix(const std::string& FILE_NAME, const int ROWS, const int COLS)
{
	...
	{ 
		...
		{
			for (int c = 0; c < COLS; c++) // for each column in a row 
			{
				file << 3 << " ";
			}
...
int menu(){
	...
	std::cout << "0.- Exit " << std::endl;
 	...
 	std::cout << "4. Divide two matrices " << std::endl;
...
void printMatrix( std::vector<std::vector<double>> aux){
	        ...
	        for (std::size_t row = 0; row < aux.size();  row++)  // for each row
            {
                for (std::size_t col = 0; col < aux[row].size(); col++)   // for each column
                {
